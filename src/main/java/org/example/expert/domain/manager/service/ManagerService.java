...
public class ManagerService {
    ...
    public ManagerSaveResponse saveManager(AuthUser authUser, long todoId, ManagerSaveRequest managerSaveRequest) {
        ...
        Todo todo = todoRepository.findById(todoId)
                .orElseThrow(() -> {
                    logService.saveLog("Todo를 찾지 못했습니다.", HttpStatus.NOT_FOUND);
                    return new InvalidRequestException("Todo not found");
                });

        if (todo.getUser() == null || !ObjectUtils.nullSafeEquals(user.getId(), todo.getUser().getId())) {
            logService.saveLog("담당자를 등록하려고 하는 유저가 유효하지 않거나, 일정을 만든 유저가 아닙니다.", HttpStatus.BAD_REQUEST);
            throw new InvalidRequestException("담당자를 등록하려고 하는 유저가 유효하지 않거나, 일정을 만든 유저가 아닙니다.");
        }

        User managerUser = userRepository.findById(managerSaveRequest.getManagerUserId())
                .orElseThrow(() -> {
                    logService.saveLog("등록하려고 하는 담당자 유저가 존재하지 않습니다.", HttpStatus.NOT_FOUND);
                    return new InvalidRequestException("등록하려고 하는 담당자 유저가 존재하지 않습니다.");
                });

        if (ObjectUtils.nullSafeEquals(user.getId(), managerUser.getId())) {
            logService.saveLog("일정 작성자는 본인을 담당자로 등록할 수 없습니다.", HttpStatus.BAD_REQUEST);
        ...
        logService.saveLog("매니저 등록에 성공했습니다.", HttpStatus.OK);
